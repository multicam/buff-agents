 ...
 * OpenRouter Provider
 * 
 * Fallback provider that routes to multiple model providers via OpenRouter API.
...
export class OpenRouterProvider implements LLMProvider {
    ...
    readonly supportedModels = [/.*/] // Supports all models as fallback
    ...
    constructor(config: LLMProviderConfig) {
        this.client = new OpenAI({
            ...
            defaultHeaders: {
                'HTTP-Referer': 'https://buff-agents.dev',
                'X-Title': 'buff-agents',
            },
        })
    }
    ...
    private normalizeModel(model: string): string {
        ...
        // Convert from our format (provider/model) to OpenRouter format
        if (model.startsWith('openrouter/')) {
            return model.replace('openrouter/', '')
        }
        // For other providers, map to OpenRouter model names
        ...
        if (model.startsWith('google/')) {
            return model.replace('google/', 'google/')
        }
        return model
    }
